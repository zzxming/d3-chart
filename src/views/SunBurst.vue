...
<script lang="ts" setup>
    ...
    let loadData = async () => {
        let res = await new Promise<Data>(async (resolve, reject) => {
            resolve(
                (await import("../assets/d3json/ZoomableSunburst.json"))
                    .default as Data
            );
        }).catch((err) => {
            console.log(err);
    ...
    onMounted(async () => {
        ...
        let color = d3.scaleOrdinal(
            d3.quantize(
                ...
                (data.value as SunDataWitchChildren).children.length + 1
            )
        );
        let arc = d3
        ...
        let partition = (data: Data) => {
            ...
            return d3.partition<Data>().size([2 * Math.PI, root.height + 1])(
                root
            );
        };
        ...
        function clicked(e: MouseEvent, p: d3.HierarchyRectangularNode<Data>) {
            ...
            path.transition(
                ...
                .filter(function (d) {
                    ...
                    return Boolean(
                        Number(self.getAttribute("fill-opacity")) ||
        ...
        function labelTransform(
            d: d3.HierarchyRectangularNode<Data>["current"]
        ) {
