...
<script lang="ts" setup>
    ...
    let tips = ref(
        (d3Tip as Function)()
            ...
            .html(
                (d: GeoJSON.Feature) =>
                    `<span class="d3_tip-name">${
                        d.properties?.name ?? "没有名字"
                    }</span>`
            )
    );
    // 加载初始省份边界数据
    let loadData = async () => {
        ...
        for (let i = 0; i < chinaMapProvinceNameData.length; i++) {
            let data = await import(
                `@/assets/d3json/chinaMap/wrap/${chinaMapProvinceNameData[i]}.json`
            );
            chinaMapData.push(data.default);
        }
        let data: GeoJSON.FeatureCollection[] | void = await Promise.all(
            chinaMapData
        ).catch((err) => {
            console.log(err);
    ...
    onMounted(async () => {
        ...
        async function loadInside(e: any, d: GeoJSON.Feature) {
            ...
            let cityData = await import(
                `@/assets/d3json/chinaMap/inside/${d.properties?.name}.json`
            );
