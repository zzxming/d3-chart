...
<script lang="ts" setup>
    ...
    let tips = ref(
        (d3Tip as Function)()
            ...
            .html(
                (_: MouseEvent, d: Data) =>
                    `<span class="d3_tip-name">${d.name ?? "没有名字"}</span>`
            )
    );
    ...
    let loadData = async () => {
        ...
        let res = await new Promise<LoadData>(async (resolve, reject) => {
            resolve(
                (
                    await import(
                        `@/assets/d3json/TheWealthAndHealthOfNations.json`
                    )
                ).default as LoadData
            );
        }).catch((err) => {
    ...
    onMounted(async () => {
        ...
        const svg = d3
            .select(".chart_box")
            .append("svg")
            .attr("viewBox", [0, 0, width - scrollWidth, height]);
        // x 为经费
        ...
        let y = d3.scaleLinear(
            [
                dataRange.value.min_paper_amount,
                dataRange.value.max_paper_amount,
            ],
        ...
        let valueAt = (datas: [Date, number][], date: Date) => {
            ...
            if (i > 0) {
                ...
                const t =
                    (date.getTime() - a[0].getTime()) /
                    (b[0].getTime() - a[0].getTime());
        ...
        const circle = svg
            ...
            .selectAll("circle")
            .data(
                dataAt(new Date(dataRange.value.min_year, 0, 1)),
                (d) => (d as DataInProcess).name
            )
            .join("circle")
        ...
        let preObservablehq =
            document.getElementsByClassName("observablehq")[0];
